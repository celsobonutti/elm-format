...
processElts refmap (L _lineNumber lf : rest) =
    case lf of
        ...
        TextLine t
            | Just terms1 <- T.stripPrefix "@docs" t ->
                let
    ...
    case ct of
        ...
        FencedCode _ _ info' ->
            singleton (CodeBlock attr txt)
                <> processElts refmap rest
          where
            txt = joinLines $ map extractText $ toList cs
                    ...
                    (cs' :> L _ (TextLine t)) ->
                        ...
                        put $
                            ContainerStack
                                ( Container
                                    ct
                                    (cs' |> L lineNumber (SetextHeader lev t))
                                )
                                rest
    ...
    newContainers = do
        ...
        if null verbatimContainers
            then (,) <$> pure regContainers <*> leaf lastLineIsText
                ...
                (,)
                    <$> pure (regContainers ++ verbatimContainers)
                    <*> textLineOrBlank
...
textLineOrBlank = consolidate <$> takeText
  where
    consolidate ts
        | T.all isWhitespace ts = BlankLine ts
...
leaf lastLineIsText =
  ...
  where
    removeATXSuffix t = case T.dropWhileEnd (`elem` (" #" :: String)) t of
        t'
            | T.null t' -> t'
            -- an escaped \#
            | T.last t' == '\\' -> t' <> "#"
...
parseHtmlBlockStart =
  ...
  where
    f (Opening name) = name `Set.member` blockHtmlTags
